...
# PBC$time is time to event in years

Surv(
     time = as.numeric(pbc$dateout),
     event = pbc$d,
     origin = as.numeric(pbc$datein)
)
...
# Survival from birth and entry in the study
Surv(
     time = whl$timein,
     time2 = whl$timeout,
     event = whl$chd,
...
Surv(
     time = whl$timein / 365.25,
     time2 = whl$timeout / 365.25,
     event = whl$chd,
...
mydata=read_csv("Datasets/surv_data_practical1.csv")
...
legend(
        ...
        1,
        c(
                expression(paste(kappa, "=2, ", lambda, "=0.2")),
                expression(paste(kappa, "=2, ", lambda, "=0.4")),
                expression(paste(kappa, "=4, ", lambda, "=0.4")),
                expression(paste(kappa, "=0.8, ", lambda, "=0.4"))
        ),
...
loglog.haz <-
        function(x, theta, kappa) {
                (exp(theta) * kappa * x ^ (kappa - 1)) / (1 + exp(theta) * x ^ (kappa))
        }

curve(loglog.haz(x, 1, 0.2),
      xlab = "Time",
      ylab = "Hazard function",
      col = "red")
